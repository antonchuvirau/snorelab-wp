  ...
  useEffect(() => {
    if (!isLoaded && allProducts.data) {
      ...
      .then((response) => {
        ...
        const _products = allProducts.data.filter((product) => {
          console.log(product);
          if (!product.acf.only_for_selected_countries
            || product.acf.product_countries.findIndex(e => e.product_country === data.country_name) !== -1
          ) {
            return true;
          } else {
            return false;
